...
| `wo multitenancy update [--force]` | Stage a core release honoring `wp_version` and compare its `$wp_db_version` with the active core. A higher staged schema triggers tenant exports before assets or the flip, then per-tenant `wp core update-db` after the flip. Equal schemas keep the fast path. An older, unreadable, or unparseable staged/active schema aborts before asset promotion because automated DB downgrades are unsupported. Any failed export also aborts. Post-flip DB failures leave the new core active, report partial/nonzero status, and require manual remediation. `--force` only skips the canary abort. |
...
After staging core, `update` parses `$wp_db_version` from both the active and staged `wp-includes/version.php`. If the staged schema is higher, it first exports every tenant with `wp db export` into a root-only timestamp directory under `shared_root/backups/db/`. Any tenant export failure aborts before shared assets, canary testing, or the release flip. These DB dumps accumulate and are the operator's responsibility to retain or prune after confirming they are no longer needed. Equal schema versions keep the existing fast update path and do not create tenant dumps. A lower staged schema aborts because automated database downgrade is unsupported; an unreadable or unparseable marker on either side also aborts rather than guessing.

After a schema upgrade is live and its release record is updated, `update` runs `wp core update-db` separately for every tenant. Failures do not trigger an automatic rollback because other tenant databases may already have migrated. The command still performs release/asset cleanup, reports each failed domain and a manual `wp core update-db --path=… --allow-root` remediation command, emits `result=partial` with the failed count and pre-upgrade backup directory, and exits nonzero.
